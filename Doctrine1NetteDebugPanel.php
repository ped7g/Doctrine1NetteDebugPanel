...
";
		...
		foreach ($queries as $event) {
			...
			$evtxt = htmlSpecialChars( $event['SQL'] );
			//de-alias columns
			...
			if ( $event['params'] !== NULL ) {
				foreach( $event['params'] as $p ) {
					...
					$ptxt = htmlSpecialChars( $p );
					$evtxt = substr_replace( $evtxt, "<span style='color:blue;'>'$ptxt'</span>", $qmarkpos, 1 );
				}
			}
			//** output all data
			$content .= "
<tr {$classes[($i=1-$i)]}>
	...
	<td>" . $event['fetched'] . "</td>
	<td>" . htmlSpecialChars( $event['conn'] ) . "</td>
</tr>
";
		}
		$content .= '</table></div>';
